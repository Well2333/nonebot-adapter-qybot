...
from .utils import decode_message_id, encode_message_id
    ...
    *,
    ...
    参数:
        ...
        auto_escape: 消息内容是否作为纯文本发送（即不解析 CQ 码），
            只在 `message` 字段是字符串时有效
    """
    if user_id and group_id and message_type == "group":
        user_id = None
    if user_id:
        return await send_private_msg(
            adapter, botId, user_id=user_id, message=message, auto_escape=auto_escape
        )
    if group_id:
        return await send_group_msg(
            adapter, botId, group_id=group_id, message=message, auto_escape=auto_escape
        )
    else:
        raise ValueError("api need user_id or group_id")
...
async def delete_msg(
    ...
    *,
    ...
    参数:
        message_id: 消息 ID
    """
    message_id_ = decode_message_id(message_id)
    if len(message_id_.split("-")) == 4:
        await adapter.post(
            path="recallFriendMessage", **{"botId": botId, "messageId": message_id_}
        )
    else:
        await adapter.post(
            path="recallGroupMessage", **{"botId": botId, "messageId": message_id_}
        )
...
async def set_group_anonymous_ban(
    ...
    *,
    group_id: int,
    anonymous: Union[Dict[str, Any], None] = ...,
    anonymous_flag: Union[str, None] = ...,
    duration: int = ...,
) -> None:
    """群组匿名用户禁言。
...
async def get_friend_list(
    ...
    """获取好友列表。"""
    friends: List = (await adapter.post("listFriend", botId=botId)).get("friends", [])
    return [
        {
...
async def get_group_list(
    ...
    """获取群列表。"""
    groups: List = (await adapter.post("listGroup", botId=botId)).get("groups", [])
    return [
        {
            "group_id": int(group["id"]),
            "group_name": group["name"],
            "member_count": int(group["total"]),
